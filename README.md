...
Release uses `LazySplit.entitlements`. For live Sign in with Apple and Plaid OAuth redirects, register `com.aayushrawal.LazySplit` in the Apple Developer portal, enable both capabilities on that App ID, point `lazysplit.aayushrawal.com` at the API, and let Xcode regenerate the provisioning profile. The API serves the Apple App Site Association document at `/.well-known/apple-app-site-association`.
