...
In **Cards & Accounts → Upload statements**, choose a CSV or PDF. Both monthly and yearly PDFs are supported. Monthly filenames such as `Apple Card - August 2026.pdf`, `Statement_2026-08.pdf`, or `08-2026.pdf` prefill the statement month; annual/yearly filenames prefill the year. A month name without a year uses the file's modification year and is clearly flagged for confirmation. PDFKit extracts digital text and Apple's on-device Vision text recognition handles pages without recognizable text rows. No raw PDF or page images leave the phone or are retained by LazySplit. Confirm the inferred statement type, period, card, and currency; expand extracted rows to correct dates/descriptions/amounts or exclude rows. CSV rows with negative amounts or descriptions identifying a payment, return, refund, credit, or adjustment are discarded before preview and never written to the app ledger. Explicit review confirmation is required before import. Imported purchases enter the review inbox and are never published automatically.
...
Chase statements use the `Opening/Closing Date` printed inside the PDF as the confirmed-period suggestion, ahead of a filename or file-date fallback. Dated filenames such as `20250815-statements.pdf` are also recognized. American Express statements receive coordinate-aware extraction because their PDFs may store dates, descriptions, and amounts in separate text columns even though they appear as visual rows. Amex posting-date asterisks and Pay Over Time footnote markers (including the modern `⧫` marker) are accepted. Foreign-currency Amex rows use the final billed amount rather than the preceding Foreign Spend value. Amex headings are treated as hints rather than required parser state: each dated row is classified independently so a missing or reordered `New Charges` heading cannot suppress the rest of a month's purchases. Payments, returns, credits, fees, interest, adjustments, and cash advances remain excluded. Issuer-specific suggestions remain visible and require review before import.
...
The app performs one complete paginated transaction download when establishing a cache, then requests only records changed since the previous successful sync. Plaid removals are delivered as tombstones so deleted or pending-replaced charges leave the phone without another full download. Foreground refreshes are coalesced, while card connection, statement import, and pull-to-refresh force an immediate incremental check. Local review decisions remain authoritative until their bounded batch upload succeeds. To keep large backfills responsive, New renders 100 charges at a time, large history sets initially expand only the newest group, search is debounced, and group totals are computed once per snapshot. Accounts shows a compact card selector and one account dashboard at a time. Its month-by-month view defaults to the last 48 months and can be changed to 12, 24, 36, or 48 months; coverage is aggregated in one pass. Each month is labeled with month/year, purchase count, and **Synced**, **Uploaded**, **Both**, or **Missing** status; a missing month opens the statement upload flow. Accounts → **Rename this account** sets a private LazySplit nickname when the bank supplies a generic/rewards-program name instead of the card product.
